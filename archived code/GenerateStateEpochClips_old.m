...
% max num epochs)
%
% looks in local_movie_root for the source videos
    ...
    for e=1:min(36, pruned_epochs(k).num_epochs)
        ...
        if localstopframe<localstartframe 
            ...
            elseif movieboundary<=round(length(absframes)/2)
                ...
                localstartframe=1;
                movie_filename=sourcemoviefilename{absstopframe};
                movie_path=sourcemoviefilepath{absstopframe};
            else error('wtf')
            end
        end
        v = VideoReader(fullfile(movie_path, movie_filename));
